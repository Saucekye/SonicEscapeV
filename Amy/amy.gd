...
@export var stored_mask: int        # Saved before entering loops
var in_loop = false
...
# ─────────────────────────────────────────────
var swipe = false       # True during the swipe attack animation window (gates animations and attackbox)
...
var default = ((-2.0 * jump_height) / (jump_time_to_descent * jump_time_to_descent)) * -1.0  # Baseline fall gravity
...
var texture = "res://Sonic/sonicsheetsonic-sheetmakeup2-sheet.png"  # Unused
...
func _physics_process(delta):
	...
	if is_on_floor():
		...
		# Higher bounce than other characters (-1200 vs -750/-950/-1100)
		# and no tier system — always the same height regardless of bounce count
...
func handle_floor_logic(delta):
	if is_on_floor():
		...
		# Ground swipe — only fires if not already mid-swipe or mid-dash
...
func handle_air_logic(delta, is_grounded):
	if not is_on_floor():
		...
		if dashed == false:
			# No flying/ouch/hang guards here — simpler than Knuckles' version
			if (falling == false or (falling == true and not ball)) and ball == false and grinding == false and not is_on_floor():
				ap.play("jump")
...
func handle_air_actions(is_grounded):
	if grinding == false:
		# ── Air Dash ──────────────────────────────────────────────────
		# Standard Sonic-style burst dash — no meter cost, one use per airtime
...
func handle_jump_input(is_grounded):
	...
	# Variable jump height — cut upward velocity on early button release
...
func handle_wall_mechanics():
	pass  # No wall interaction for this character — intentionally empty
	# NOTE: Wall climb logic exists below in a block comment if needed in future

	# (Wall climb/jump code is commented out in source — preserved below for reference)
	# To re-enable: uncomment the block and remove the pass above
...
func update_animations():
	...
	if ouch == false:
		# Don't override animation while touching a wall (reserved for future wall states)
		...
		if is_on_floor():
			...
			ap.speed_scale = 1  # No flying state to protect here (unlike Knuckles)
	else:
		ap.play("hurt")
...
func handle_movement_animations(doturn):
	if doturn < 1 and doturn > -1 and doturn == abs(doturn) and grinding == false:
		...
		if ball != true and grinding == false and swipe == false:  # swipe blocks movement anims
...
# ─────────────────────────────────────────────
func dash(direction):
	# Standard air dash — identical to Sonic's implementation
	falling = true
	dashed = true
	ball = false
	crouch = false
	...
	ap.play("flick")
	motion.y = -450
	fall_gravity = 0        # Gravity disabled briefly for hang-time feel
	smokeemit()
	sfx.pitch_scale = 2
	sfx.stream = load("res://Sonic Sfx/SA_113.wav")
	sfx.play()
...
func airdown():
	# Stomp — note: no flying = false here (no flying state on this character)
	# Also no motion.x = 0 — horizontal momentum is NOT cancelled on stomp (differs from others)
	bounce += 1
	next_bounce = true
	falling = true
	dashed = true
...
func airspin():
	# Swipe attack — same 0.25s window as Tails' version
	# No meter cost and no flying cancellation (no flying state on this character)
	swipe = true
	dashed = true
	ball = false
	crouch = false
...
func airup():
	# Currently unused — no input triggers this in handle_air_actions (commented out)
	dashed = true
	ball = false
	if is_player == true:
		Test.meter -= 50
...
func peelout():
	# Charge-only — no release logic implemented
...
func _on_hitbox_area_entered(area: Area2D) -> void:
	...
	if area.is_in_group("Rail"):
		# No flying state to reset here (unlike Tails/Knuckles)
		grinding = true
		bounce = 0
		can_dash = true
		dashed = false
