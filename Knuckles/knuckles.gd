...
@export var stored_layer: int       # Collision layer saved before entering loops
@export var stored_mask: int        # Collision mask saved before entering loops
var in_loop = false
...
# ─────────────────────────────────────────────
# Knuckles-Unique State Variables
# ─────────────────────────────────────────────
...
var flying = false                  # True while gliding (Knuckles glides, doesn't fly like Tails)
...
var can_dash = true
var dashx = false                   # Delays friction after a dash
var dashed = false
var crouch = false
var spin = 0                        # Unused
var ball = false
...
var last_trick = ""
var can_stomp = true
var bounce = 0                      # Consecutive stomp counter
...
var default = ((-2.0 * jump_height) / (jump_time_to_descent * jump_time_to_descent)) * -1.0  # Baseline fall gravity; restored after special moves
...
var texture = "res://Sonic/sonicsheetsonic-sheetmakeup2-sheet.png"  # Unused texture path
var stickdir = Vector2(0,0)
@export var player_path: NodePath
var player: CharacterBody2D

# ─────────────────────────────────────────────
# Hang System (carried by a flying player)
# ─────────────────────────────────────────────
var hang = false
var hangable = false
...
func _physics_process(delta):
	...
	# Drain meter and check drill exit conditions each frame while drilling
	if is_drilling:
	...
	if is_on_floor():
		...
		# Capture real velocity when leaving floor so slope momentum carries into air
	...
	if abs(motion.x) > 1250:
		...
		if time_elapsed > 50:
			if max_speed == 500 && is_on_floor():
				...
				$Sfx.pitch_scale = 2
				$Sfx.stream = load("res://Sonic Sfx/Break Speed.wav")
				$Sfx.play()
				smokeemit()
	...
	# ── AI Follow Logic ────────────────────────────────────────────────
	# Wider stop_range than other characters (100) — Knuckles hangs back a bit more
	...
	# ── Wall Stop ──────────────────────────────────────────────────────
	# Note: only stops movement if NOT drilling (drilling ignores walls)
...
func throw_item():
	...
	if sfx:
		sfx.pitch_scale = 1.2
		sfx.stream = load("res://Sonic Sfx/SA_113.wav")
		sfx.play()
...
func handle_air_logic(delta, is_grounded):
	if not is_on_floor():
		...
		if dashed == false:
			# Don't override with jump animation while gliding, hurt, or on wall
			if (falling == false or (falling == true and not ball)) and ball == false and grinding == false and flying == false and not hang and not ouch and not is_on_wall() and not is_on_floor():
				ap.play("jump")
...
func handle_air_actions(is_grounded):
	if grinding == false:
		...
		if is_player == true and Test.meter >= 50 and not Input.is_action_pressed("ui_down") and (not Input.is_action_pressed("ui_up") or Input.is_action_pressed("ui_down")) and Input.is_action_just_pressed("airspin") and direction != 0:
			airspin()
			ap.play("airspin")
			time_elapsed = 100      # Lower momentum boost than Sonic (100 vs 200)
			max_speed = 1300
			can_dash = true
			can_stomp = true
			bounce = 0
...
func handle_jump_input(is_grounded):
	...
	if Input.is_action_just_pressed("ui_accept") and (grinding or is_grounded or is_coyote_time_active()) and (ball or (!crouch and !ball)) and not just_wall_jumped and not is_spinning:
		...
		if !Input.is_action_pressed("ui_down") or can_jump_with_down:
			...
			# Extra guards vs Sonic: also suppresses jump anim during hurt, hang, or wall contact
...
# ─────────────────────────────────────────────
# Wall Mechanics (Knuckles-unique: climb instead of wall-jump)
# Knuckles can cling to walls, climb up/down, and wall-jump from them.
# ─────────────────────────────────────────────
func handle_wall_mechanics():
	if ($CollisionShape2D/WallCast.is_colliding() or $CollisionShape2D/WallCast2.is_colliding()) and (not $CollisionShape2D/Raycast.is_colliding()) and rot == 0:
		...
		print("ON WALL - Current animation: ", ap.current_animation)
		...
		ball = false
		crouch = false
		flying = false  # Glide ends on wall contact (transitions to climb)

		# ── Wall Climbing ──────────────────────────────────────────────
		if Input.is_action_pressed("ui_up"):
			motion.y = -280         # Climb upward
			ap.play("climb")
			ap.speed_scale = 1.0
		elif Input.is_action_pressed("ui_down"):
			motion.y = 280          # Slide down
			ap.play("climb")
			ap.speed_scale = -1.0   # Reverse animation when sliding down
		else:
			motion.y = 0            # Hold in place
			ap.play("climb")
			ap.speed_scale = 0.0    # Freeze animation when idle on wall
...
func dash(direction):
	...
	falling = false     # Not falling while gliding
	ball = false
	crouch = false
	flying = true       # Glide flag — triggers handle_glide_physics every frame
	can_dash = false

	motion.y = clamp(motion.y, -200, 100)  # Gentle starting descent
	fall_gravity = 80                        # Much lower than default — nearly floating
	...
	if abs(motion.x) < 1100:
		...
		motion.x = 1100 * glide_direction
		max_speed = 850   # Lower cap during glide to keep it controlled
		acc = 3000
		time_elapsed = 60

func airdown():
	# Stomp — cancels glide and slams straight down
	bounce += 1
	next_bounce = true
	falling = true
	dashed = true
	can_dash = true
	flying = false      # Stomp cancels the glide
	time_elapsed = 0
	motion.y = 1000
	ap.play("stomp")
	fall_gravity = 10500
	sfx.pitch_scale = 2
	sfx.stream = load("res://Sonic Sfx/Spiked.wav")
	sfx.play()
	motion.x = 0
	await get_tree().create_timer(0.13).timeout
	fall_gravity = default
...
func airspin():
	# Air spin also activates wall drilling if there's enough meter.
	# The drill lets Knuckles pass through walls while spinning.
	falling = true
	dashed = true
	ball = false
	var meter_cost = 50
	if is_player == true:
		if Test.meter >= meter_cost:
			...
			start_wall_drilling()   # Enable collision pass-through
		else:
			pass  # Not enough meter — spin without drilling
	ap.play("airspin")
	motion.y = -400      # Less upward force than Sonic's airspin (-650)
	fall_gravity = 0
	smokeemit()
	max_speed = 1200
	acc = 5000
...
func spinaudio():
	...
	if random_audio:
		...
		sfx.stream = load("res://Sonic Sfx/Trick.wav")
		sfx.pitch_scale = 2
		sfx.play()
		voice.play()

# ─────────────────────────────────────────────
# Wall Drilling System (Knuckles-unique)
# Temporarily disables a collision layer so Knuckles can pass through
# walls while spinning. Drains meter per second while active.
# Automatically stops when no wall geometry is detected nearby.
# ─────────────────────────────────────────────
var wall_collision_layer = 1  # The layer index representing walls to drill through

func start_wall_drilling():
	is_drilling = true
	drill_start_time = Time.get_ticks_msec() / 1000.0
	original_collision_mask = collision_mask
	# Remove the wall layer bit from the collision mask to phase through walls
	collision_mask &= ~(1 << wall_collision_layer)
	print("Started drilling - disabled layer ", wall_collision_layer)

func handle_wall_drilling(delta):
	# Drain meter while drilling
	...
	# Cast rays in 6 directions to check if still inside or near wall geometry
	# If no walls are found, exit drill mode automatically
	var space_state = get_world_2d().direct_space_state
	var test_offsets = [
		Vector2(500, 0),
		Vector2(-500, 0),
	...
	# NOTE: smoke effect code is unreachable here due to the early return above
	# TODO: move smokeemit() call before the return, or restructure

	# Allow directional control while drilling through a wall
	if direction != 0:
		motion.x = 400 * direction
	else:
		motion.x = move_toward(motion.x, 0, 1000 * delta)

	# Vertical movement while drilling is currently commented out
	# Uncomment to allow up/down drilling:
	#	if Input.is_action_pressed("ui_up"):
	#		motion.y = -400
	#	elif Input.is_action_pressed("ui_down"):
	#		motion.y = 400
...
func spindash():
	if is_on_floor() and Input.is_action_pressed("ui_down") and ball == false and next_bounce == false and motion.x == 0 and direction == 0:
		...
		if Input.is_action_just_pressed("ui_accept"):
			...
			$Sfx.stream = load("res://Sonic Sfx/rev.MP3")
			$Sfx.play()

	elif is_on_floor() and Input.is_action_just_released("ui_down") and is_spinning:
		$Sfx.pitch_scale = 1.5
		$Sfx.stream = load("res://Sonic Sfx/spindash.MP3")
		$Sfx.play()
		is_spinning = false
		crouch = true
		ball = true
...
func peelout():
	# Simplified — only charges, no release logic implemented
...
func _on_hitbox_area_entered(area: Area2D) -> void:
	if area.is_in_group("Rings"):
		if ouch == false:
			$Sfx.stream = load("res://obstacles/rings/ringsfx.MP3")
			$Sfx.pitch_scale = 1
			$Sfx.play()
...
func hurt():
	if is_player == true:
		...
		if Test.meter <= 0:
			Test.meter = 0
	$Sfx.pitch_scale = 1
	$Sfx.stream = load("res://Sonic Sfx/sonic-rings-drop.MP3")
	$Sfx.play()
	ouch = true
	motion = Vector2(0, 0)
	time_elapsed = 0
